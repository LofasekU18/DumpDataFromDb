...
try
{
    ...
    List<ExekucniPrikazy> listData = await GetListOfExeAsync(connectionString, query, token);
...
{
    Console.WriteLine("Query was canceled by user.");
}
catch (Exception ex)
{
...
async Task<List<ExekucniPrikazy>> GetListOfExeAsync(string connectionString, string query, CancellationToken token)
{
